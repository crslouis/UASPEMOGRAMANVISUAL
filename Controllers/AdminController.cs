...
namespace GrandLineBooks.Controllers
{
    public class AdminController : Controller
    {
        ...
        public async Task<IActionResult> CustomerReport()
        {
            ...
            var reportData = new List<CustomerReportItem>();
            foreach (var order in orders)
            {
                var customer = await _mongoDBService.Users
                    .Find(u => u.Id == order.CustomerId)
                    .FirstOrDefaultAsync();

                if (customer != null)
                {
                    reportData.Add(new CustomerReportItem
                    {
            ...
            ViewBag.TotalRevenue = reportData.Sum(r => r.TotalPayment);
            ViewBag.TotalOrders = reportData.Count;

            return View(reportData);
        }

        // GET: Admin/PaymentReport
        public async Task<IActionResult> PaymentReport(DateTime? startDate, DateTime? endDate)
        {
            if (!IsAdmin())
                return RedirectToAction("Login", "Account");

            // Base filter: only include orders that are considered "paid"
            var filter = Builders<Order>.Filter.Eq(o => o.Status, "Delivered");

            if (startDate.HasValue)
            {
                var start = startDate.Value.Date;
                filter &= Builders<Order>.Filter.Gte(o => o.OrderDate, start);
            }
            ...
            if (endDate.HasValue)
            {
                ...
                filter &= Builders<Order>.Filter.Lte(o => o.OrderDate, end);
            }

            var paidOrders = await _mongoDBService.Orders
                .Find(filter)
                .SortByDescending(o => o.OrderDate)
                .ToListAsync();

            var paymentReport = new List<CustomerReportItem>();
            ...
            foreach (var order in paidOrders)
            {
                ...
                if (customer != null)
                {
                    paymentReport.Add(new CustomerReportItem
                    {
                        CustomerName = customer.FullName,
                        OrderDate = order.OrderDate,
                        TotalPayment = order.TotalAmount,
            ...
            ViewBag.TotalRevenue = paymentReport.Sum(r => r.TotalPayment);
            ViewBag.TotalOrders = paymentReport.Count;
